...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  signWinBinaries: () => {
    ...
    fs.copySync(path.join(config.outputDir, 'fulldive.exe'), path.join(dir, 'fulldive.exe'));
  ...
  generateWidevineSigFiles: () => {
    ...
    util.run('python', [sig_generator, '--input_file=' + path.join(src_dir, 'fulldive.exe'),
        '--flags=1',
        '--certificate=' + cert,
        '--private_key=' + key,
        '--output_file=' + path.join(config.outputDir, 'fulldive.exe.sig'),
