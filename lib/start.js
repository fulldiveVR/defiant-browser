...
const start = (passthroughArgs, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (!outputPath) {
    if (process.platform === 'darwin') {
      ...
      let appName = 'Fulldive\\ Browser\\ Development'

      if (buildConfig === 'Release') {
        appName = 'Fulldive\\ Browser'
      }
      ...
      outputPath = path.join(config.outputDir, 'fulldive.exe')
    } else {
      outputPath = path.join(config.outputDir, 'fulldive')
    }
  }
